    ...
    class WC_Tech_Netpay extends WC_Payment_Gateway {
        ...
        public function generate_netpay_form($order_id)
        {
            ...
            $netpay_args = array(
                ...
                'description' => $this->mcrypt_encrypt_cbc(trim(substr($order_description, 0, 100)), $this->enc_key, $this->enc_iv),
            ...
            if (trim($order->get_formatted_shipping_address()) == '') {
                $netpay_info_args = array(
                    'bill_to_company' => preg_replace('/[\x00-\x1F\x80-\xFF]/', '', trim(substr(strip_tags($order->billing_company), 0, 100))),
                    'bill_to_address' => preg_replace('/[\x00-\x1F\x80-\xFF]/', '', trim(substr(strip_tags($order->billing_address_1 . ' ' . $order->billing_address_2), 0, 100))),
                    'bill_to_town_city' => preg_replace('/[\x00-\x1F\x80-\xFF]/', '', trim(substr(strip_tags($order->billing_city), 0, 50))),
                    'bill_to_county' => preg_replace('/[\x00-\x1F\x80-\xFF]/', '', trim(substr(strip_tags($order->billing_state), 0, 50))),
                    'bill_to_postcode' => preg_replace('/[\x00-\x1F\x80-\xFF]/', '', trim(substr(strip_tags($order->billing_postcode), 0, 10))),
                    'bill_to_country' => preg_replace('/[\x00-\x1F\x80-\xFF]/', '', trim(substr(strip_tags($this->getValidCountryCode($order->billing_country)), 0, 3))),
                    ...
                    'ship_to_firstname' => preg_replace('/[\x00-\x1F\x80-\xFF]/', '', trim(substr(strip_tags($order->billing_first_name), 0, 50))),
                    'ship_to_lastname' => preg_replace('/[\x00-\x1F\x80-\xFF]/', '', trim(substr(strip_tags($order->billing_last_name), 0, 50))),
                    'ship_to_fullname' => preg_replace('/[\x00-\x1F\x80-\xFF]/', '', trim(substr(strip_tags($order->billing_first_name . ' ' . $order->billing_last_name), 0, 100))),
                    'ship_to_company' => preg_replace('/[\x00-\x1F\x80-\xFF]/', '', trim(substr(strip_tags($order->billing_company), 0, 100))),
                    'ship_to_address' => preg_replace('/[\x00-\x1F\x80-\xFF]/', '', trim(substr(strip_tags($order->billing_address_1 . ' ' . $order->billing_address_2), 0, 100))),
                    'ship_to_town_city' => preg_replace('/[\x00-\x1F\x80-\xFF]/', '', trim(substr(strip_tags($order->billing_city), 0, 50))),
                    'ship_to_county' => preg_replace('/[\x00-\x1F\x80-\xFF]/', '', trim(substr(strip_tags($order->billing_state), 0, 50))),
                    'ship_to_country' => preg_replace('/[\x00-\x1F\x80-\xFF]/', '', trim(substr(strip_tags($this->getValidCountryCode($order->billing_country)), 0, 3))),
                    'ship_to_postcode' => preg_replace('/[\x00-\x1F\x80-\xFF]/', '', trim(substr(strip_tags($order->billing_postcode), 0, 10))),
                    'ship_to_method' => trim(substr($order->shipping_method, 0, 20)),
            ...
            //Otherwise use billing and shipping address supplied stripping out any unwanted characters
                ...
                $netpay_info_args = array(
                    ...
                    'customer_email' => preg_replace('/[\x00-\x1F\x80-\xFF]/', '', strip_tags($order->billing_email)),
                    'customer_phone' => preg_replace('/[^0-9]/', '', strip_tags($order->billing_phone)),
                    'ship_to_firstname' => preg_replace('/[\x00-\x1F\x80-\xFF]/', '', trim(substr(strip_tags($order->shipping_first_name), 0, 50))),
                    'ship_to_lastname' => preg_replace('/[\x00-\x1F\x80-\xFF]/', '', trim(substr(strip_tags($order->shipping_last_name), 0, 50))),
                    'ship_to_fullname' => preg_replace('/[\x00-\x1F\x80-\xFF]/', '', trim(substr(strip_tags($order->shipping_first_name . ' ' . $order->shipping_last_name), 0, 100))),
                    'ship_to_company' => preg_replace('/[\x00-\x1F\x80-\xFF]/', '', trim(substr(strip_tags($order->shipping_company), 0, 100))),
                    'ship_to_address' => preg_replace('/[\x00-\x1F\x80-\xFF]/', '', trim(substr(strip_tags($order->shipping_address_1 . ' ' . $order->shipping_address_2), 0, 100))),
                    'ship_to_town_city' => preg_replace('/[\x00-\x1F\x80-\xFF]/', '', trim(substr(strip_tags($order->shipping_city), 0, 50))),
                    'ship_to_county' => preg_replace('/[\x00-\x1F\x80-\xFF]/', '', trim(substr(strip_tags($order->shipping_state), 0, 50))),
                    'ship_to_country' => preg_replace('/[\x00-\x1F\x80-\xFF]/', '', trim(substr(strip_tags($this->getValidCountryCode($order->shipping_country)), 0, 3))),
                    'ship_to_postcode' => preg_replace('/[\x00-\x1F\x80-\xFF]/', '', trim(substr(strip_tags($order->shipping_postcode), 0, 10))),
                    'ship_to_method' => trim(substr($order->shipping_method, 0, 20)),
                    'customer_ip_address' => trim(substr($_SERVER['REMOTE_ADDR'], 0, 15)),
                    'customer_hostname' => trim(substr($_SERVER['HTTP_HOST'], 0, 60)),
                    'customer_browser' => preg_replace('/[\x00-\x1F\x80-\xFF]/', '', trim(substr(strip_tags($browserName['name']), 0, 60))),
